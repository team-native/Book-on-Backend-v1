...
const rememberDlsUserKey = async (req: Request, userKey: string) => {
  const query = notificationQueries.updateUserDlsKey(req.userId!, userKey);
...
export const getCurrentLoan = async (req: Request, res: Response) => {
  ...
  await rememberDlsUserKey(req, userKey);
